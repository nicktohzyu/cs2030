...
class BusService {
    ...
    public CompletableFuture<Set<BusStop>> getBusStops() {
        CompletableFuture<Scanner> sc = BusAPI.getBusStopsServedBy(serviceId).thenApply(s -> new Scanner(s));
        CompletableFuture<Set<BusStop>> stops = sc.thenApply(s -> s.useDelimiter("\n")
                .tokens()
                .map(line -> line.split(","))
